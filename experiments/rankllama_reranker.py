...
def _load_rankllama(
    ...
    base_model = AutoModelForSequenceClassification.from_pretrained(
        config.base_model_name_or_path, num_labels=1
    )
...
class RankLlamaReranker(Transformer):
    ...
    A appliquer sur le top-k d'un premier retrieval (ex. `bm25 % 20 >> RankLlamaReranker()`).
    """
    ...
    def _model(self) -> PreTrainedModel:
        return _load_rankllama(self.peft_model_name).to(self._device)
...
rankllama_reranker: Transformer = RankLlamaReranker(verbose=True)